...
class LukaServiceInfoEX(Poll, Converter, object):
	apid = 0
	vpid = 1
	sid = 2
	onid = 3
	...
	ENCRYPTION_TYPE = 58
	SUBTITLE_INFO = 59
	MEDIA_INFO = 60  # New merged item for Resolution, VideoCodec, and AudioCodec  # added by MNASR
	VSIZE_INFO = 61  # New item for vsize, video codec, and audio codec  # added by MNASR
	IS_DECRYPTED = 62  # added by MNASR
	...
	def __init__(self, type):
		...
		elif type == "IsDecrypted":
			self.type = self.IS_DECRYPTED		
		elif type == "IsFta":
	...
	def getBoolean(self):
		...
		elif self.type == self.IS_DECRYPTED:
			return info.getInfo(iServiceInformation.sVideoPID) == 1	
